...
function NotMobile({ handleLogin, isUserLoggedIn }) {
    ...
    return (
        <div className="not-mobile">
            {!isUserLoggedIn ? (
                <button className="login-button" onClick={toggleMenu}>Login</button>
            ) : (
            ...
            {isMenuOpen && (
                <div ref={menuRef} className={isUserLoggedIn ? "nm-user-true" : "nm-user-false"}>
                    {isUserLoggedIn ? (
                        <>
                            <Link to="/profile" className="nm-link2" onClick={closeMenu}><FaUser className="fa-icon"/>Profile</Link>
                        ...
                        <>
                            <Link className="nm-link1" onClick={() => {closeMenu(), handleLogin()}}>Log in</Link>
